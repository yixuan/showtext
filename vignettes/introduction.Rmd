...
```{r echo=FALSE, fig.align='center', fig.showtext=TRUE}
showtext.end()
...
```r
font.add(family = "<font_name>", regular = "/path/to/font/file")
```

where `family` is the name that you assign to that font (so that later you can
call `par(family = "<font_name>")` to use this font in plotting),
...
font.add("constan", regular = "constan.ttf", italic = "constani.ttf")
...
difficulty in using the alternatives.
...
through the function `font.add.google()`, as the first example shows.
